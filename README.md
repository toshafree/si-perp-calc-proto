...
Все заменяемые бизнес-константы собраны в [`src/config/dealConfig.ts`](src/config/dealConfig.ts): дата экспирации, ГО инструментов, комиссия, начальный капитал, ставка займа, множитель и пресеты фандинга.
...
- ГО по позиции по умолчанию: `пары × max(fut_margin, perp_margin)`; значение можно изменить вручную.
